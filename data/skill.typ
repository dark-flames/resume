...
#let skillList(env) = (
  (
    ...
    content: multiLang(
      env,
      en: [
        Not limited to any specific language, especially experienced in Agda, Haskell, PHP, Python, Rust, comfortable with C/C++, Java, JavaScript, OCaml, TypeScript. Skilled in metaprogramming, code generation, and type-driven development.
      ],
      cn: [
        可以使用任何编程语言进行工作，精通 Agda、Haskell、PHP、Python、Rust，熟悉 C/C++、Java、JavaScript、OCaml、TypeScript （排名均不分先后）。擅长元编程，代码生成与类型驱动开发。
      ]
    )
  ),
  (
    ...
    content: multiLang(
      ...
      cn: [
        熟悉各种依值类型论和他们的语义模型，具有形式化验证的经验，熟练使用 Agda 等定理证明器。
      ]
    )
  ),
  (
    ...
    cv-content: true,
    content: multiLang(env, en: [
      understand various compiler architectures, familiar with compiler frontend, especially in type checking. Also have experience in parser generator and DSL design.
    ], cn: [
      了解主流编程语言的设计，理解各种编译器架构，熟悉编译器前端，尤其是类型检查和静态分析。擅长使用各种 parser 生成器，具有 DSL 以及编程语言工具链的设计与实现经验。
    ])
  ),
  (
    ...
    content: cv-and-others(
      ...
      multiLang(env, en :[
        ...
        *7年*全栈 Web 开发经验，精通后端开发、数据库和 DevOps 实践。
      ]), multiLang(env, en: [
  ...
  (
    ...
    cv-content: false,
    content: multiLang(env, en: [
        can adapt to any editors/OSs, usually use JetBrains IDEs and VSCode under macOS, familiar with git and docker, and have experience with team collaboration tools like GitHub and Slack.
      ], cn: [
        能适应任何编辑器与操作系统，通常在 macOS 下使用 JetBrains IDE 和 VSCode，熟悉 git 和 docker，并有使用 GitHub 和 Slack 等团队协作工具的经验。
      ])
  ),
  (
    ...
    cv-content: true,
    content: multiLang(env, en: [
        Chinese: native, English: advanced, Japanese: intermediate
      ], cn: [
        汉语：母语，英语：高级，日语：中级
      ])
  ),
